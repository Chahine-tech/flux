 ...
 * `Activity.make` around the exact same `@flux/application` use cases.
 * There is no bridge here: an `Activity`'s `execute` is an
 ...
 * of re-running — a sharp, non-obvious edge found empirically, so
